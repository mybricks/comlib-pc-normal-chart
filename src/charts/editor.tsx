...
// function legendChange (input, value) {
//   Object.keys(inputIdMap).map(key => {
//     const inputId = inputIdMap[key]
//     const inputPin = input.get(inputId)
//     if (inputId.split('.').includes('legend')) {
//       if (value) {
//         if (!inputPin){
//           input.add(inputId, inputSchemaMap[inputId].title, inputSchemaMap[inputId].schema)
//         }
...
export default {
  ...
  ':root': ({ data }: EditorResult<Data>, cate0, cate1) => {
    cate0.title = '常规';
    cate0.items = [
      {
        title: '类型',
...

      {
        ...
        items: [
          ...
          },
          {
            ...
            ifVisible({ data }: EditorResult<Data>) {
              return data.type !== chartTypes.PIE && data.type !== chartTypes.BIDIRECTIONAL_BAR && data.type !== chartTypes.DUAL_AXES
            },
            value: {
              get({ data }: EditorResult<Data>) {
          ...
          },
          {
            ...
            description: '横轴映射对应的数据字段名',
            ifVisible({ data }: EditorResult<Data>) {
              return data.type === chartTypes.BIDIRECTIONAL_BAR || data.type === chartTypes.DUAL_AXES
            },
            value: {
              get({ data }: EditorResult<Data>) {
                return Array.isArray(data.config.yField) ? data.config.yField.join(',') : data.config.yField;
              },
              set({ data, input }: EditorResult<Data>, value: string) {
      ...
      // 图例
      {
        ...
        items: [
          ...
          //   type: 'code',
          //   ifVisible({ data }) {
          //     return data.config.legend === 'custom';
          //   },
          //   options: {
          //     theme: 'light',
          //     title: '自定义图例',
          ...
          //   value: {
          //     get({ data }: EditorResult<Data>) {
          //       return data.config.customLegend || defaultCustomFn;
          //     },
          //     set({ data }: EditorResult<Data>, value: string) {
          //       data.config.customLegend = value;
          //     },
          //   },
          // },
        ],
      ...
      //       value: {
      //         get({ data }: EditorResult<Data>) {
      //           return data.customToolTips || defaultCustomFn;
      //         },
      //         set({ data }: EditorResult<Data>, value: string) {
      //           data.customToolTips = value;
      //         },
      //       },
      //     },
      //   ],
